...
// Uses provider APIs instead of rclone to check remote metadata and download changes.
// Creates SyncChangelog entries so the shared processChangelog workflow handles versioning.
...
// Resolver-backed entity for downloading files directly from cloud providers
// using provider-specific APIs (e.g., Microsoft Graph for OneDrive).
// Used by native file picker flows that bypass rclone.

entity CloudDownload {
    provider String,
    connectionId String,
    items String,
...
resolver cloudDownloadResolver [knowledge.core/CloudDownload] {
    create r.createCloudDownload
}

// --- Native picker sync ---
// Resolver-backed entity for periodic sync of native file picker connections.
// Checks remote metadata via provider APIs, downloads changed files to staging,
// and creates SyncChangelog entries for processChangelog to handle.
