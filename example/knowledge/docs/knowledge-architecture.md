...
- **Cloud connections** — OAuth-based access to OneDrive, Dropbox, Box, and Google Drive via rclone or native provider APIs
- **Automated sync** — Scheduled and on-demand one-way sync from cloud storage into the platform, supporting both rclone-based directory sync and native file picker per-document sync
...
Connections have a `syncMode` field (`"rclone"` or `"native_picker"`) that determines which sync path is used. The `syncTick` scheduler routes each connection to the appropriate workflow based on its `syncMode`.

#### rclone sync (syncMode: "rclone")

Used for connections created via rclone remote configuration — typically folder-level sync.

```
1. Scheduler (or manual trigger) → syncConnection workflow
...
#### Native picker sync (syncMode: "native_picker")

Used for Google Drive and OneDrive connections where files were selected via native file pickers. Instead of rclone, the resolver calls provider APIs directly to check remote metadata and download changed files.

```
1. Scheduler → syncNativePickerConnection workflow
2. Knowledge Service creates SyncJob record (status: in_progress)
3. NativePickerSync resolver:
   a. Gets access token via integration-manager
   b. Queries all non-deleted KnowledgeDocuments for the connection
   c. For each document, fetches remote metadata via provider API:
      - Google Drive: GET /drive/v3/files/{fileId}?fields=modifiedTime,size,trashed
      - OneDrive: GET /v1.0/{remotePath}?$select=lastModifiedDateTime,size,deleted
   d. If file is trashed/deleted/404 → creates "deleted" SyncChangelog entry
   e. If remoteModifiedAt is newer → downloads file to staging, creates "modified" SyncChangelog entry
   f. If unchanged → skips
4. processChangelog workflow handles versioning (same as rclone path)
5. SyncJob updated with status and error (if any)
```
...
entity Connection {
    ...
    syncMode @enum("rclone", "native_picker") @default("rclone"),
...
For connections created via native file pickers (Google Drive, OneDrive), the service uses provider APIs directly instead of rclone to check for remote changes and download updated files. A `NativePickerSync` resolver-backed entity encapsulates this logic:

```
entity NativePickerSync {
    connectionId String,
    tenantId String,
    provider String,
    externalConnectionId String,
...
resolver nativePickerSyncResolver [knowledge.core/NativePickerSync] {
    create r.syncNativePicker
}
```

The resolver (`syncNativePicker`):

1. Gets an OAuth access token via `agentlang.getAccessToken()` (managed by integration-manager)
2. Queries all non-deleted `KnowledgeDocument` records for the connection via REST self-call
3. For each document, fetches remote metadata using provider-specific APIs:
   - **Google Drive:** `GET https://www.googleapis.com/drive/v3/files/{fileId}?fields=modifiedTime,size,trashed`
   - **OneDrive:** `GET https://graph.microsoft.com/v1.0/{remotePath}?$select=lastModifiedDateTime,size,deleted`
4. If the file is trashed/deleted (or returns 404), creates a `SyncChangelog` entry with `changeType: "deleted"`
5. If `remoteModifiedAt` is newer than what's stored, downloads the file to `{stagingDir}/{externalConnectionId}/{remotePath}`, sets the file's mtime via `utimesSync`, and creates a `SyncChangelog` entry with `changeType: "modified"`
6. If unchanged, skips the document
7. Returns `syncStatus`, `errorMessage`, and `errorIncrement` on the instance

The staging path layout (`{stagingDir}/{externalConnectionId}/{remotePath}`) matches what `storeVersion` expects, so the shared `processChangelog` workflow handles versioning without modification.

Per-document errors are logged and skipped (the sync continues). Connection-level errors (token failure, document query failure) fail the entire sync.
...
```
// Timer callback. Route connections to the appropriate sync workflow based on syncMode.
workflow syncTick {
    // rclone-backed connections
    {Connection {syncEnabled? true, status? "ready", syncMode? "rclone"}} @as rcloneConns;
    for conn in rcloneConns {
    ...
    // Native picker connections (Google Drive, OneDrive)
    {Connection {syncEnabled? true, status? "ready", syncMode? "native_picker"}} @as pickerConns;
    for conn in pickerConns {
        {SyncJob {connectionId? conn.id, status? "in_progress"}}
            @empty {syncNativePickerConnection {connectionId conn.id,
                                                tenantId conn.tenantId,
...
// Full sync lifecycle for one rclone-backed connection:
...
// Sync lifecycle for native picker connections (Google Drive, OneDrive).
// Uses provider APIs instead of rclone. Creates SyncChangelog entries
// so the shared processChangelog workflow handles versioning.
workflow syncNativePickerConnection {
    {SyncJob {connectionId syncNativePickerConnection.connectionId,
              tenantId syncNativePickerConnection.tenantId,
              status "in_progress", trigger "scheduled",
              startedAt now()}} @as [job];

    {NativePickerSync {connectionId syncNativePickerConnection.connectionId,
                       tenantId syncNativePickerConnection.tenantId,
                       provider syncNativePickerConnection.provider,
                       externalConnectionId syncNativePickerConnection.externalConnectionId,
                       syncJobId job.id}} @as [result];

    {processChangelog {syncJobId job.id,
                       tenantId syncNativePickerConnection.tenantId,
                       connectionId syncNativePickerConnection.connectionId,
                       remoteName syncNativePickerConnection.externalConnectionId}};

    {SyncJob {id? job.id,
              status result.syncStatus,
              errorMessage result.errorMessage,
              completedAt now()}};
    {Connection {id? syncNativePickerConnection.connectionId,
                 syncErrorCount syncErrorCount + result.errorIncrement}}
}
...
2. **Route by syncMode** — `syncTick` queries connections in two passes: first `syncMode: "rclone"` connections, then `syncMode: "native_picker"` connections. Each pass uses its own sync workflow.
3. **Concurrency guard** — for each ready connection, queries for an existing `in_progress` SyncJob; `@empty` ensures the sync workflow only runs if no sync is already active

**rclone path** (`syncConnection`):

4. **SyncJob created** — creates a SyncJob with status `in_progress`
5. **rclone sync** — the `CloudFileProxy` resolver uses a stable per-remote staging directory (`stagingDir/remoteName`) so rclone can perform incremental syncs, calls `sync/sync`, catches errors internally, and returns `stagingPath`, `syncStatus`, `errorMessage`, and `errorIncrement` on the instance
6. **Changelog — added** — the `StagingFile` resolver walks the staging directory. For each file not matching a `KnowledgeDocument`, a `SyncChangelog` entry with `changeType: "added"` is created
7. **Changelog — modified** — for each staging file with a different `remoteModifiedAt` than the DB record, a "modified" entry is created
8. **Changelog — deleted** — for each `KnowledgeDocument` with no matching staging file, a "deleted" entry is created

**Native picker path** (`syncNativePickerConnection`):

4. **SyncJob created** — creates a SyncJob with status `in_progress`
5. **NativePickerSync resolver** — gets an OAuth access token via `agentlang.getAccessToken()`, queries all non-deleted `KnowledgeDocument` records for the connection, then for each document fetches remote metadata via provider API (Google Drive or Microsoft Graph). If the file is trashed/deleted/404, creates a "deleted" `SyncChangelog` entry. If `remoteModifiedAt` is newer, downloads the file to `{stagingDir}/{externalConnectionId}/{remotePath}`, sets mtime, and creates a "modified" entry. Unchanged files are skipped.

**Shared version management** (`processChangelog`, runs for both paths):

9. **Version management — added** — copies staging file to versioned store via `VersionStore` resolver, creates `KnowledgeDocument` (version 1) + `DocumentVersion`, increments SyncJob counters
10. **Version management — modified** — copies file to store, bumps `currentVersion`, creates new `DocumentVersion`, increments counters
11. **Version management — deleted** — soft-deletes `KnowledgeDocument` (`isDeleted: true`), increments `filesDeleted`
12. **SyncJob updated** — writes final status, error message, and completion timestamp
13. **Error tracking** — `syncErrorCount` on the Connection is incremented by `errorIncrement` (0 for success, 1 for failure)
