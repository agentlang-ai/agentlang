...
function evaluateOrAnd(orAnd: OrAnd, env: Environment): Result {
  switch (orAnd.op) {
    case 'or':
      return evaluateOr(orAnd.exprs, env);
    case 'and':
      return evaluateAnd(orAnd.exprs, env);
    default:
...
function evaluateOr(exprs: LogicalExpression[], env: Environment): Result {
  for (let i = 0; i < exprs.length; ++i) {
    if (evaluateLogicalExpression(exprs[i], env)) return true;
  }
  return false;
}
...
function evaluateAnd(exprs: LogicalExpression[], env: Environment): Result {
  for (let i = 0; i < exprs.length; ++i) {
    if (!evaluateLogicalExpression(exprs[i], env)) return false;
  }
  return true;
}
