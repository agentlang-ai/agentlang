...
let serverUrl = 'https://mcp.deepwiki.com/mcp';
...
function printHelp(): void {
  ...
  console.log(
    '  call-tool-task <name> [args] - Call a tool with task-based execution (example: call-tool-task delay {"duration":3000})'
  );
  console.log('  greet [name]               - Call the greet tool');
  console.log('  multi-greet [name]         - Call the multi-greet tool with notifications');
  console.log(
    '  collect-info [type]        - Test form elicitation with collect-user-info tool (contact/preferences/feedback)'
  );
  ...
  try {
    ...
    if (toolsResult.tools.length === 0) {
      ...
      for (const tool of toolsResult.tools) {
        console.log(
          `  - id: ${tool.name}, name: ${getDisplayName(tool)}, description: ${tool.description}`
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (promptsResult.prompts.length === 0) {
      ...
      for (const prompt of promptsResult.prompts) {
        console.log(
          `  - id: ${prompt.name}, name: ${getDisplayName(prompt)}, description: ${prompt.description}`
        );
      }
    }
  } catch (error) {
...
async function getPrompt(name: string, args: Record<string, unknown>): Promise<void> {
  ...
  try {
    ...
    promptResult.messages.forEach((msg, index) => {
      console.log(
        `  [${index + 1}] ${msg.role}: ${msg.content.type === 'text' ? msg.content.text : JSON.stringify(msg.content)}`
      );
    });
  } catch (error) {
  ...
  try {
    ...
    if (resourcesResult.resources.length === 0) {
      ...
      for (const resource of resourcesResult.resources) {
        console.log(
          `  - id: ${resource.name}, name: ${getDisplayName(resource)}, description: ${resource.uri}`
        );
      }
    }
  } catch (error) {
      ...
      switch (message.type) {
        ...
        case 'taskStatus':
          if (lastStatus !== message.task.status) {
            console.log(
              `  ${message.task.status}${message.task.statusMessage ? ` - ${message.task.statusMessage}` : ''}`
            );
          }
