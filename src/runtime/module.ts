  ...
  if (moduleDb == undefined) {
    if (useReactiveModuleDbFlag) {
      moduleDb = z.create(set => ({
        ...
        adModule: (name: string) =>
          set((state: any) => ({ db: state.db.set(name, new RuntimeModule(name)) })),
        removeModule: (name: string) => set((state: any) => ({ db: state.db.delete(name) })),
        set: (name: string, mod: RuntimeModule) =>
          set((state: any) => ({ db: state.db.set(name, mod) })),
      }));
    } else {
...
export function getModuleNames(): string[] {
  const ks: Iterable<string> = useReactiveModuleDbFlag
    ? moduleDb((state: any) => state.db.keys())
    : moduleDb.keys();
...
export function getAllEventNames() {
  ...
  const modDb: Map<string, RuntimeModule> = useReactiveModuleDbFlag
    ? moduleDb((state: any) => {
        return state.db;
      })
    : moduleDb;
