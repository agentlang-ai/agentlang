...
import {
  executeEventHelper,
  executeStatement as executeStatement,
} from '../../src/runtime/exec-graph.js';
import {
  Instance,
  isInstanceOfType,
  makeInstance,
...
describe('Basic exec-graph evaluation', () => {
  ...
  test('basic-agents', async () => {
    if (process.env.AL_TEST === 'true') {
      ...
      const r01: Instance = await executeStatement(
        `{exg02/personManager {message "create Joe with email joe@acme.com"}}`
      );
