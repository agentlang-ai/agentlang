...
import { Path } from '../../src/runtime/util.js';
...
describe('Issue 97', () => {
  test('test01', async () => {
    ...
    const cra = async (resource: Instance, name: string): Promise<Instance> => {
      const r: any =
        await parseAndEvaluateStatement(`{I97/Resource {id? "${resource.lookup('id')}"},
    ...
    let result: any[] = await parseAndEvaluateStatement(
      `{I97/FetchResourceAllocations {id "${r01.lookup('id')}"}}`
    );
    assert(result.length == 2);
    const chk2 = (result: any[]) =>
      assert(
        result.every((obj: any) => {
          return obj.e == 'r01' && (obj.t == 'a01' || obj.t == 'a02');
        })
      );
    chk2(result);
    ...
    const r = result.filter((r: any) => {
      return r.e == 'r02';
    });
    assert(r.length == 1);
    assert(r[0].t == 'a03');
    await parseAndEvaluateStatement(
      `{I97/FetchAllocationsResource {id "${a01.lookup('id')}"}}`
    ).then((result: any[]) => {
      assert(result.length == 1);
      assert(result[0].e == 'r01' && result[0].t == 'a01');
    });
    await parseAndEvaluateStatement(
      `{I97/FetchAllocationsResource {id "${a03.lookup('id')}"}}`
    ).then((result: any[]) => {
      assert(result.length == 1);
    ...
    isa(
      await parseAndEvaluateStatement(
        `{I97/CreateAllocation {id "${r02.lookup('id')}", name "a04"}}`
      )
    );
    await parseAndEvaluateStatement(
      `{I97/FetchResourceAllocations {id "${r02.lookup('id')}"}}`
    ).then((result: any[]) => {
      assert(result.length == 2);
      assert(
        result.every((r: any) => {
          return r.e == 'r02' && (r.t == 'a03' || r.t == 'a04') && (r.r === 0 || r.r === 0.234);
        })
      );
    });
  });
...
describe('Issue 97 (contains)', () => {
  test('test01', async () => {
    ...
    const cra = async (resource: Instance, name: string): Promise<Instance> => {
      const r: any =
        await parseAndEvaluateStatement(`{I97C/Resource {id? "${resource.lookup('id')}"},
    ...
    const a03 = await cra(r02, 'a03');
    let result: any[] = await parseAndEvaluateStatement(
      `{I97C/FetchResourceAllocations {id "${r01.lookup('id')}"}}`
    );
    assert(result.length == 2);
    const chk2 = (result: any[]) =>
      assert(
    ...
    assert(r[0].t == 'a03');
    await parseAndEvaluateStatement(
      `{I97C/FetchAllocationsResource {id "${a01.lookup('id')}"}}`
    ).then((result: any[]) => {
      assert(result.length == 1);
      assert(result[0].e == 'r01' && result[0].t == 'a01');
    });
    await parseAndEvaluateStatement(
      `{I97C/FetchAllocationsResource {id "${a03.lookup('id')}"}}`
    ).then((result: any[]) => {
      assert(result.length == 1);
      assert(result[0].e == 'r02' && result[0].t == 'a03');
    });
...
if (isUsingSqlite()) {
  // Postgres will rollback transaction on SQL error
  describe('Issue-197', () => {
    test('Catch handler should execute', async () => {
      ...
      const chk = async (n: string) => {
        await parseAndEvaluateStatement(`{I197/${n}? {}}`).then((result: Instance[]) => {
          assert(result.length == 2);
          const ids = result.map((inst: Instance) => {
            return inst.lookup('id');
          });
          assert(
            ids.find((v: number) => {
              return v == 1;
...
`
    ...
    const rfas = async (
      ...
      const res: Instance[] = await parseAndEvaluateStatement(
        `{I209/${event} {StartDate "${start}", EndDate "${end}"}}`
      );
      return res;
    };
...
`
    ...
    const getra = async (resId: number, intoResultCount?: number) => {
      const rs = await parseAndEvaluateStatement(
        `{I233/GetResourceAllocation${intoResultCount ? '2' : '1'} {Id ${resId}}}`
      );
      const rrs: any = intoResultCount
        ? rs.map((x: object) => {
    ...
    const getar1 = async (id: number, resId: number) => {
      const rs: Instance[] = await parseAndEvaluateStatement(
        `{I233/GetOneAllocationResource {Id ${id}}}`
      );
      assert(rs.length == 1);
      chkresId(rs[0], resId);
    };
...
describe('rel-alias-order', () => {
  test('relationship aliases must not be swapped', async () => {
    const moduleName = 'RelAlias';
    await doInternModule(
      moduleName,
      `entity A {
                id Int @id
    ...
    const f = (name: string, type: string, from: any, to: any) => {
      const sourcePath = new Path(from.module, from.entity);
      const targetPath = new Path(to.module, to.entity);
      const node1: RelationshipNode = {
        path: sourcePath,
        alias: from.entity,
        origName: sourcePath.asFqName(),
        origAlias: from.entity,
      };

      const node2: RelationshipNode = {
        path: targetPath,
        alias: to.entity,
        origName: targetPath.asFqName(),
        origAlias: to.entity,
      };
      if (type === 'contains') {
        addContainsRelationship(name, moduleName, [node1, node2]);
      }
      if (type === 'between') {
...
relationship AB1 between (RelAlias/A @as A, RelAlias/B @as B)

relationship AB2 contains (RelAlias/A @as A, RelAlias/B @as B)
