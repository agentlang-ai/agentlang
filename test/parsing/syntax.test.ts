...
} from '../../src/runtime/agents/common.js';
...
describe('Pattern generation using the syntax API', () => {
  test('test01', async () => {
    ...
    crud4
      .addAttribute('age?', LiteralPattern.Array([age1, age2]), 'between')
...
UserProfile {Profile {email CreateUser.email}}}`;
    ...
    const ifp: IfPattern = new IfPattern(new ExpressionPattern('emp.salary > 1000'))
      ...
      .setElse([
        new IfPattern(new ExpressionPattern('emp.salary < 500'))
          .addPattern(LiteralPattern.String('-500'))
...
UserProfile {Profile {email CreateUser.email}},UserPost {Post {title "hello, world"}}}`
    ...
    assert(
      qup.toString() == '{User {Age?> 18, Status 3}}',
      'Failed to regenereate query-update pattern'
    );
    ...
    assert(qup.isQueryUpdate, 'Failed to detect query-update pattern');
    assert(
      qup.toString() == '{User {Age?between [10, 20], Status 3}}',
      'Failed to regenereate query-update pattern'
    );
    ...
    assert(qup.isQueryUpdate, 'Failed to detect query-update pattern');
    assert(
      qup.toString() == '{User {Name?like "Th%", Status 3}}',
      'Failed to regenereate query-update pattern'
    );
    ...
    const subArray = LiteralPattern.Array([
      LiteralPattern.Id('a'),
      LiteralPattern.Reference('a.b'),
    ]);
    const arrayPat = LiteralPattern.Array([
      LiteralPattern.Number(100),
      LiteralPattern.String('hi'),
      subArray,
    ]);
    assert(arrayPat.toString() == '[100, "hi", [a, a.b]]');

    const mapPat = LiteralPattern.Map(
      new Map().set({ str: 'a' }, LiteralPattern.Number(1)).set({ str: 'b' }, arrayPat)
    );
...
describe('agent-xtras-to-string', () => {
  test('standalone scenarios, directives', async () => {
    ...
    const cond1 = new IfPattern(LiteralPattern.String('Sam hits jackpot')).addPattern(
      LiteralPattern.Id('GuidedAgent/scenario01')
    );
...
describe('case-generation', () => {
  test('decsion-cases generated from pattern objects', async () => {
    const c1 = new CasePattern(
      LiteralPattern.String('salary is greater than 1000'),
      LiteralPattern.Id('Accept')
    );
...
describe('directive-generation', () => {
  test('directives generated from pattern objects', async () => {
    const cond1 = new IfPattern(LiteralPattern.String('salary > 1000')).addPattern(
      LiteralPattern.String('accept the offer')
    );
...
describe('scenario-generation', () => {
  test('scenarios generated from pattern objects', async () => {
    const cond1 = new IfPattern(LiteralPattern.String('salary > 1000')).addPattern(
      LiteralPattern.Id('acme.core/incrementSalary')
    );
...
describe('import-syntax', () => {
  test('import statements in module.toString', async () => {
    const mname = 'ImpSyn';
    await doInternModule(
      mname,
...
`
    ...
    m.addImport({ name: 'AI', path: './modules/ai.ts' });
    const s2 = m.toString();
    assert(
      s2 ===
        `module ImpSyn
